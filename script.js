...
const adicionar = () => { // Arrow Fuction responsável por adicionar tarefas é criada.
  ...
  if(document.getElementById('nomeTarefa').value != '') { // Esse if testa se o campo de texto foi preenchido para só então executar as funcionalidades da função.
    ...
    obj.grau = document.getElementById('grau').value; // Sua importância é definido.
  ...
  novoItem.innerHTML = `<strong>Tarefa:</strong> ${obj.nome.toUpperCase()}. <strong>Dia a ser concluída:</strong> ${obj.data}. <strong>Importância:</strong> ${obj.grau.toUpperCase()} <br /> <strong>Descrição:</strong> ${obj.descricao}`; // Aqui as informações que aparecerão no site são organizadas.
...
const editarTarefa = () => { // Arrow Function que edita a tarefa escolhida
  ...
  for(let i = 0; i < listaTarefas.length; i++) /** Procura o elemento que será editado  */ {
    if(listaTarefas[i].nome == x) /** SE o elemento for encontrado, será editado */{
      ...
      novoItem.innerHTML = `<strong>Tarefa:</strong> ${x.toUpperCase()}. <strong>Dia a ser concluída:</strong> ${listaTarefas[i].data}. <strong>Importância:</strong> ${listaTarefas[i].grau.toUpperCase()} <br /> <strong>Descrição:</strong> ${listaTarefas[i].descricao}`; // Aqui as informações que aparecerão no site são organizadas.
...
const concluida = (x) => {
  ...
  for(let i = 0; i < listaTarefas.length; i++) /** Procura o elemento que será editado  */{
    if(listaTarefas[i].nome === x) /** Esse if testa se o campo de texto foi preenchido para só então executar as funcionalidades da função. */ {
      ...
      /* Os campos de input serão limpos */
